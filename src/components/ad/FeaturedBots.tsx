    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3">
        {bots.map((bot) => (
          <div
            key={bot.id}
            className="p-3 bg-card border border-border rounded-xl flex items-center gap-3 hover:border-primary/30 transition-all group"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground truncate font-mono opacity-60">
                @{bot.username}
              </p>
            </div>
            <div className="text-right">
              <div className="text-xs font-black text-primary tabular-nums">
                {(bot.activeUsers30d || 0) >= 1000
                  ? `${((bot.activeUsers30d || 0) / 1000).toFixed(1)}K`
                  : bot.activeUsers30d || 0}
              </div>
              <div className="text-[8px] text-muted-foreground uppercase font-bold opacity-50">
                {fb?.users ?? "Active Users"}
              </div>
            </div>
          </div>
        ))}
