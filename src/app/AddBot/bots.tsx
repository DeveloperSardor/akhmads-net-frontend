...
const Bots = () => {
  ...
  const handleDelete = async (botId: string, botUsername: string) => {
    const confirmMsg = (b?.deleteConfirm ?? "Are you sure you want to delete @{{username}}?")
    ...
    <div className="mt-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-white/60 mt-1">
            {botsList.length} {botsList.length === 1 ? b?.botRegistered : b?.botsRegistered}
          </p>
        </div>
        <button
      ...
      {botsList.length === 0 ? (
        <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.03] to-white/[0.01] p-16 text-center">
          ...
          <p className="text-sm text-white/50 max-w-md mx-auto">{b?.noBotsDesc}</p>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10 bg-white/[0.02]">
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-white/60">{b?.tableHeaders.bot}</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-white/60">{b?.tableHeaders.performance}</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-white/60">{b?.tableHeaders.earnings}</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-white/60">{b?.tableHeaders.status}</th>
                  <th className="px-6 py-4 text-right text-xs font-semibold uppercase tracking-wider text-white/60">{b?.tableHeaders.actions}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {botsList.map((bot) => (
                  <tr key={bot.id} className="transition hover:bg-white/[0.03] group">
                    {/* Bot Info */}
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-white/80">
                            {formatNumber(bot.impressionsServed || 0)} {b?.views}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-xs">
                          <TrendingUp className="w-3 h-3 text-blue-400" />
                          <span className="text-white/80">{bot.ctr || 0}% CTR</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="font-bold text-green-400">{formatCurrency(bot.totalEarnings || 0)}</p>
                          <p className="text-xs text-white/50">
                            {formatCurrency(bot.pendingEarnings || 0)} {b?.pending}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-2">
                        <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium ${getStatusColor(bot.status)}`}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          onClick={() => navigate(`/${lang}/bots/${bot.id}/settings`)}
          ...
          <div className="border-t border-white/10 bg-white/[0.01] px-6 py-4">
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-6 text-white/60">
                <span>{b?.totalBots}: <strong className="text-white">{botsList.length}</strong></span>
                <span className="w-px h-4 bg-white/10"></span>
                <span>
                  {b?.totalEarnings}: <strong className="text-green-400">
                    {formatCurrency(botsList.reduce((sum, bot) => sum + (bot.totalEarnings || 0), 0))}
                  </strong>
                </span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4 backdrop-blur-sm">
          <div className="relative w-full max-w-lg rounded-2xl border border-white/10 bg-[#0A0A0A] p-6 shadow-2xl">
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div className="flex items-center gap-3">
                <img 
                  src={`${API_BASE_URL}/bots/avatar/${selectedBot.username}`} 
                ...
                />
                <div>
                  <h3 className="text-lg font-bold text-white">@{selectedBot.username}</h3>
                  <p className="text-sm text-white/50">{selectedBot.firstName}</p>
                </div>
              </div>
              <button
            ...
            <div className="mt-6 space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm font-medium ${getStatusColor(selectedBot.status)}`}>
              ...
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-lg font-bold text-white">{formatNumber(selectedBot.totalMembers || 0)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/50">Faol a'zolar</p>
                    <p className="text-lg font-bold text-purple-400">{formatNumber(selectedBot.activeMembers || 0)}</p>
                  </div>
                </div>
                {selectedBot.botstatData && (
                  <div className="mt-4 grid grid-cols-3 gap-2 border-t border-white/5 pt-3">
                    <div className="text-center rounded-lg bg-green-500/10 py-2 border border-green-500/20">
                      <p className="text-[10px] text-green-400 uppercase">Live User</p>
                      <p className="font-mono text-sm font-medium text-green-300">{formatNumber(selectedBot.botstatData.users_live || 0)}</p>
                    </div>
                    <div className="text-center rounded-lg bg-red-500/10 py-2 border border-red-500/20">
                      <p className="text-[10px] text-red-400 uppercase">Dead User</p>
                      <p className="font-mono text-sm font-medium text-red-300">{formatNumber(selectedBot.botstatData.users_die || 0)}</p>
                    </div>
                    <div className="text-center rounded-lg bg-blue-500/10 py-2 border border-blue-500/20">
                      <p className="text-[10px] text-blue-400 uppercase">Groups</p>
                      <p className="font-mono text-sm font-medium text-blue-300">{formatNumber(selectedBot.botstatData.groups_live || 0)}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                  <p className="mb-1 text-xs text-white/50">Ko'rishlar (Impressions)</p>
                ...
                <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                  <p className="mb-1 text-xs text-white/50">CTR (Clicktrough Rate)</p>
                  <p className="flex items-center gap-2 text-xl font-bold text-white">
              ...
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4 space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-white">{selectedBot.category}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-white/50">Bot tili</span>
                  <span className="text-sm font-medium text-white uppercase">{selectedBot.language}</span>
                </div>
                {selectedBot.shortDescription && (
                  <div className="border-t border-white/5 pt-3">
                    <span className="block text-xs text-white/50 mb-1">Tavsif</span>
                    <p className="text-sm text-white/80">{selectedBot.shortDescription}</p>
                  </div>
                )}
              </div>
            </div>
